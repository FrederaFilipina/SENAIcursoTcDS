...
export const Home = ({ setDataLogin }: Propiedades) => {
    const [ddsUsuarios, setDdsUsuarios] = useState<any>(null)
    const deslogar = () => {
        localStorage.clear()
        setDataLogin(null)
    }

    const ddsUsuario: any = jwtDecode(localStorage.getItem('tokenAcesso') || "");

    const buscarDdsUsuario = async () => {
        try {
    ...
    return (
        <div className='flex gap-10 flex-col bg-gray-400 w-full pt-32 items-center h-screen'>
            <h1 className='text-2xl font-semibold text-white'>Boas-vindas!{ddsUsuario.nome}</h1>
            <h4 className='text-sm font-ligth  text-white'>{ddsUsuario.email}</h4>
